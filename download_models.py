...
for model in models:
    print(f"Downloading {model}...")
    file_path = hf_hub_download(repo_id=repo_id, filename=model, cache_dir=script_dir, use_auth_token=access_token)
    os.rename(file_path, os.path.join(script_dir, model))
    print(f"{model} downloaded successfully!")

print("All models have been downloaded and saved to the same directory as the script.")
